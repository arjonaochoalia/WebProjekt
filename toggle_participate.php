...
//event_id we get from the form (the participate button is in a form)
...
if ($result->num_rows > 0) {
    ...
    $update = $conn->prepare("UPDATE user_events SET is_participating = ? WHERE user_id = ? AND event_id = ?");
    $update->bind_param("iii", $new_status, $user_id, $event_id);
    $update->execute();
} else {
    // User does not participate, so add participation (toggle on)
    $insert = $conn->prepare("INSERT INTO user_events (user_id, event_id, is_participating) VALUES (?, ?, 1)");
